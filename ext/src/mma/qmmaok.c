...
K1(mlgetexpr){
  ...
  switch (tag){
  case WSTKINT:v=kj(0);DBG("1\n");r=WSGetInteger64(ml_lp,&v->j);if(!r)ONWSERR;R r?v:kj(0);
  ...
  case WSTKFUNC:
    ...
    if(!r){
      ...
      /*
      tagh=WSGetNextRaw(ml_lp);
      r1=WSGetArgCount(ml_lp,&lenh);
      DBG("tagh=%d,lenh=%d\n",tagh,lenh);
      r2=WSGetSymbol(ml_lp,&s);
      vh=knk(0);
  ...
  case WSTKERROR:
  default:
      DBG("WSTKERROR:[%d](%s)!\n",WSError(ml_lp),WSErrorMessage(ml_lp));    
    break;
  }
  DBG("10\n");
  R knk(0); 
