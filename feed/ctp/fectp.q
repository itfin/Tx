...
.upd.ordnew:{[x]if[x[`sym]<>.conf.me;:.ha.ordnew[x]];k:x`oid;if[count opt:x`ordopt;h:strdict opt;if[`COMB~h`special;:.upd.combnew[x]]];if[not null .db.O[k;`sym];:()];k1:newidl[];.db.O[k;`feoid`ntime`status`x0`ft`ts`acc`fe`acc1`ref`sym`side`posefct`tif`typ`qty`price`ordopt]:(k1;.z.P;.enum`PENDING_NEW;enlist .enum.nulldict,`FrontID`SessionID!.ctrl.ctp[`FrontID`SessionID]),x`ft`ts`acc`sym`acc1`ref`osym`side`posefct`tif`typ`qty`price`ordopt;if[not (1b~.ctrl.ctp`LoginT)&(.conf.ctp.ordmax>count .db.O);rejectord[k;1i;"CTP_Not_Ready_Or_Toomany_Orders"];:()];esym:fs2s x`osym;ctpcall[`orderInsert;(.conf.ctp`broker`user),(esym;k1;$[x[`side]=.enum`BUY;.enum`D_Buy;.enum`D_Sell];`int$x`qty;$[(0<x[`price])|(esym like "SP*");.enum`OPT_LimitPrice;.enum`OPT_AnyPrice];x`price;$[x[`tif] in .enum`IMMEDIATE_OR_CANCEL`FILL_OR_KILL;.enum`TC_IOC;.enum`TC_GFD];$[x[`tif]=.enum`FILL_OR_KILL;.enum`VC_CV;.enum`VC_AV];`$ctpposefct[x[`posefct];fs2e x`osym];`$.enum`HF_Speculation)];}'; 
