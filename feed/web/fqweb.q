...
.timer.qweb:{[x]d:.z.D;t:.z.T;if[(5<={x-`week$x}d)|(d in .conf.holiday)|not any t within/:.conf.fq.timerrange;:()];fl:exec sym from .db.QX where (sym like "*XSHG")|(sym like "*XSHE");ht:.conf`webhqtype;if[0=count .temp.d:d:@[webhq_tx;fl;()];:()];linfo[`nrec;count d];d1:(d0:delete pc,open,time from d) except .temp.Last;.temp.Last:d0;if[n:count d2:select from d where sym in exec sym from d1;pub[`quote;select sym,bid,ask,bsize,asize,price,high,low,vwap,cumqty,openint,settlepx,mode:`,extime:.z.D+time,bidQ:flip (bid;b2px;b3px;b4px;b5px),askQ:flip (ask;a2px;a3px;a4px;a5px),bsizeQ:flip (bsize;b2sz;b3sz;b4sz;b5sz),asizeQ:flip (asize;a2sz;a3sz;a4sz;a5sz),quoopt:n#enlist "" from d2]];d1:(d0:select sym,pc,open from d) except .temp.LastRef;.temp.LastRef:d0;if[n:count d2:select from d0 where sym in exec sym from d1;pub[`quoteref;update sup:0w,inf:-0w,refopt:n#enlist "" each i from d2]];};
...
//webhq_tx_1:{[x]r:httpget1["qt.gtimg.cn";"/q=","," sv {[x]$[x like "*XSHG";"sh";"sz"],6#string x} each x];t:flip `sym`typ`name`code`price`pc`open`cumqty1`waipan`neipan`bid`bsize`b2px`b2sz`b3px`b3sz`b4px`b4sz`b5px`b5sz`ask`asize`o2px`o2sz`o3px`o3sz`o4px`o4sz`o5px`o5sz`bilist`dtime`zd`zdf`high`low`snap`cumqty`cumamt`hsl`pe`nop`h1`l1`zhenf`ltsz`zsz`pb`sup`inf`nop1!flip {[x](`$x[4+til 6],".",$[x[3]="h";"XSHG";"XSHE"]),"~" vs 11_x} each ";" vs -1_ r except "\"\n";select sym,"F"$pc,"F"$open,`time$"Z"${(8#x),"T",8_x} each dtime,"F"$price,("F"$cumqty)*1e2,vwap:("F"$cumamt)%("F"$cumqty),"F"$high,"F"$low,"F"$o5px,("F"$o5sz)*1e2,"F"$o4px,("F"$o4sz)*1e2,"F"$o3px,("F"$o3sz)*1e2,"F"$o2px,("F"$o2sz)*1e2,"F"$ask,("F"$asize)*1e2,"F"$bid,("F"$bsize)*1e2,"F"$b2px,("F"$b2sz)*1e2,"F"$b3px,("F"$b3sz)*1e2,"F"$b4px,("F"$b4sz)*1e2,"F"$b5px,("F"$b5sz)*1e2,openint:0n,settlepx:0n from t}; /腾讯行情[一次只能最多取64个代码]:http://qt.gtimg.cn/q=sh510310,sz000559,sz000913,sz002048,sz002085,sz002126,sz002284,sz002434,sz002472,sz002488


webhq_tx_1:{[x]r:httpget1["qt.gtimg.cn";"/q=","," sv {[x]$[x like "*XSHG";"sh";"sz"],6#string x} each x];t:flip `id`mkt`name`esym`price`pc`open`cumqty`wp`np`bid`bsize`b2px`b2sz`b3px`b3sz`b4px`b4sz`b5px`b5sz`ask`asize`a2px`a2sz`a3px`a3sz`a4px`a4sz`a5px`a5sz`un`dtime`change`chgratio`high`low`cumamt!37#flip  {(`$x[0]),vs["~"] x[1]} each refind0[r;"(.+?)=\"([^\"]+?)\";\\n"];select sym:esym {sv[`](`$x),(`1`51!`XSHG`XSHE)`$y}' mkt,"F"$pc,"F"$open,time:`time$"Z"${(8#x),"T",8_x} each dtime,"F"$price,("F"$cumqty)*1e2,vwap:("F"$last each vs["/"] each cumamt)%(1e2*"F"$cumqty),"F"$high,"F"$low,"F"$a5px,("F"$a5sz)*1e2,"F"$a4px,("F"$a4sz)*1e2,"F"$a3px,("F"$a3sz)*1e2,"F"$a2px,("F"$a2sz)*1e2,"F"$ask,("F"$asize)*1e2,"F"$bid,("F"$bsize)*1e2,"F"$b2px,("F"$b2sz)*1e2,"F"$b3px,("F"$b3sz)*1e2,"F"$b4px,("F"$b4sz)*1e2,"F"$b5px,("F"$b5sz)*1e2,openint:0n,settlepx:0n from t}; /腾讯行情[一次只能最多取900个代码]:http://qt.gtimg.cn/q=sz000559,sz000913,sz002048,sz002085,sz002126,sz002284,sz002434,sz002472,sz002488

webhq_tx:{[x]raze webhq_tx_1 each 0N 900#x};
