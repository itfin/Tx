...
onsockmsg:{[x;y].temp.X:(x;y);if[.conf.dumpmsg;.temp.B,:y];w:x`w;b:$[null w;"";.ctrl.tcpconn[w;`rbuf]],y;r:decodeszv5binstream b;if[not null w;$[0>r;[lerr[`fqxshe_decode;(r;sublist[1000]b)];if[0<r1:first ss[b] `char$0x0 vs  300111i;lerr[`fqxshe_cutmsg;r1#b];(` sv .conf.tempdb,.conf.me,`$"msg_",ssr[;":";"_"] string .z.Z) set r1#b;.ctrl.tcpconn[w;`rbuf]:r1 _b]];.ctrl.tcpconn[w;`rbuf]:r _b]];}; /hclose[.ctrl.tcpconn[w;`h]];.ctrl.tcpconn[w;`h`status`rbuf`sbuf]:(-1;`Disconnected;"";"")
...
getrd:{[](get_securityswitch[] uj get_cashauctionparams[] uj get_securities[]) uj @[get_hkexreff04;();()]};
...
loadpcfsz:{[x]m:@[getrefxml;f:`$"pcf_",string fs2s x;()];if[0=count[m];lwarn[`loadpcfsz;(x;f)];:()];r:(!/)flip m[0;1];d:"D"$r`TradingDay;.db.ETF[x;`trday`cu`cueu`cutc`cunv`cudv`nav`ctmax`rdmax`cashrmax`pubok`ctok`rdok`cashct`pfnumlocal`pfnum`pretrday`underlying`etftype`name`companyname`version]:(d;"F"$r`CreationRedemptionUnit;"F"$r`EstimateCashComponent;0n;"F"$r`NAVperCU;"F"$r`DividendPerCU;"F"$r`NAV;"F"$r`CreationLimit;"F"$r`RedemptionLimit;"F"$r`MaxCashRatio;"B"$r`Publish;"B"$r`Creation;"B"$r`Redemption;0b;"J"$r`RecordNum;"J"$r`TotalRecordNum;"D"$r`PreTradingDay;sv[`;(`$r`UnderlyingSecurityID),`XSHE];`1;`;`;`$r`Version);.db.ETF[x;`crsym]:x;delete from `.db.ETFPF where etfsym=x;.db.ETFPF,:select etfsym:x,sym:{`$x,$[x like "m*";".XDCE";x like "[acnpsz]*";".XSGE";x like "6*";".XSHG";".XSHE"]} each UnderlyingSecurityID,qty:"F"$ComponentShare,cctype:first each SubstituteFlag,ccprem:"F"$PremiumRatio,camt:"F"$CreationCashSubstitute,ramt:"F"$RedemptionCashSubstitute,trday:d from {(!/) flip x[1]} each r`Components;};

loadpcfsh:{[x]y:fs2se[x];c:`char$@[read1;f:` sv (.conf.refpathsh),`$(string y[0]),(-4#(string .z.D) except "."),".etf";()];if[0=count[c];linfo[`loadpcfsh;(x;f)];:loadpcfsh2[x]];i0:1+c?"\n";i1:first c ss "TAGTAG";i2:first c ss "ENDENDEND";c:(i0,i1,i2)_c;h:(!/)"S=\n" 0: ssr [;"maxCashratio";"MaxCashRatio"] ssr [;"Estimatecashcomponent";"EstimateCashComponent"] ssr[;"Recordnum";"RecordNum"] c[0] except "\r";.db.ETF[x;`trday`cu`cueu`cutc`cunv`cudv`nav`ctmax`rdmax`cashrmax`pubok`ctok`rdok`cashct`pfnumlocal`pfnum`pretrday`underlying`etftype`name`companyname`version]:"DFFFFFFFFFBBBBIIDSSSSS"$' h[`TradingDay`CreationRedemptionUnit`EstimateCashComponent`CashComponent`NAVperCU`DividendPerCU`NAV`CreationLimit`RedemptionLimit`MaxCashRatio`Publish`CreationRedemption`CreationRedemption`CashCreation`RecordNum`TotalRecordNum`PreTradingDay`UnderlyingIndex`Type`FundName`FundManagementCompany`Version];.db.ETF[x;`crsym]:`$h[`Fundid1],".XSHG";delete from `.db.ETFPF where etfsym=x;.db.ETFPF,:`etfsym xcols update etfsym:x,sym:{[x]se2fs x,$[(x like "6*")|(x like "11[03]*")|(x like "13*");`XSHG;`XSHE]} each sym,ramt:camt^ramt,trday:.db.ETF[x;`trday] from flip `sym`qty`cctype`ccprem`camt`ramt!("S FCFFF";"|") 0: (8_c[1]);};

loadpcfsh2:{[x]y:fs2se[x];c:`char$@[read1;f:` sv (.conf.refpathsh),`$(string y[0]),(-4#(string .z.D) except "."),"2.etf";()];if[0=count[c];lwarn[`loadpcfsh2;(x;f)];:()];i0:0;i1:first c ss "TAGTAG";i2:first c ss "ENDENDEND";c:(i0,i1,i2)_c;h:(!/)"S=\n" 0: ssr [;"maxCashratio";"MaxCashRatio"] ssr [;"Estimatecashcomponent";"EstimateCashComponent"] ssr[;"Recordnum";"RecordNum"] c[0] except "\r";.db.ETF[x;`trday`cu`cueu`cutc`cunv`cudv`nav`ctmax`rdmax`cashrmax`pubok`ctok`rdok`cashct`pfnumlocal`pfnum`pretrday`underlying`etftype`name`companyname`version]:"DFFFFFFFFFBBBBIIDSSSSS"$' h[`TradingDay`CreationRedemptionUnit`EstimateCashComponent`CashComponent`NAVperCU`DividendPerCU`NAV`CreationLimit`RedemptionLimit`MaxCashRatio`Publish`CreationRedemption`CreationRedemption`CashCreation`RecordNum`TotalRecordNum`PreTradingDay`UnderlyingIndex`Type`FundName`FundManagementCompany`Version];.db.ETF[x;`crsym]:`$h[`Fundid1],".XSHG";delete from `.db.ETFPF where etfsym=x;.db.ETFPF,:`etfsym xcols update etfsym:x,sym:{[x]se2fs x,$[(x like "6*")|(x like "11[03]*")|(x like "13*");`XSHG;`XSHE]} each sym,ramt:camt,trday:.db.ETF[x;`trday] from flip `sym`qty`cctype`ccprem`camt!("S FCF F   ";"|") 0: (8_c[1]);};
