...
BIN[309011i]:{[x].temp.X3:x;if[null z:snapshot[x];:()];if[0>=n:`long$0x0 sv `byte$x[65+til 4];:()];{[z;x]r:first each (2 8;"sj") 1:x;t:r[0];r[1]*:1e-6;$[t in `3`xa`xb`xc`xd;.db.QX[z;(`3`xa`xb`xc`xd!`price`pc`open`high`low) t]:r[1];()]}[z] each (n,10)#69_x;}; /指数快照
...
BINSS[`M102]:{[x].temp.X1:x;zh:snapshotss[x];if[null z:first zh;:()];n:`long$0x0 sv `byte$x[71+til 2];.db.QX[z;`bidQ`askQ`bsizeQ`asizeQ]:4#enlist `float$();.db.QX[z;`bid`ask`bsize`asize]:4#0n;ii:zh[1;4] in `MD001;x:73_x;if[0=count x;:()];{[ii;z;x]r:first each $[ii;(2 8;"sj");(2 8 8 1;"sjjx")] 1:x;t:r[0];r[1]*:1e-5;if[not ii;r[2]*:1.];$[t in `0`1;[l:`long$r[3];if[l=0;.db.QX[z;((`0`0;`1`0)!(`bid`bsize;`ask`asize))t,`$string l]:r[1 2]];k:$[t=`0;`bidQ;`askQ];if[l>=count d:.db.QX[z;k];.db.QX[z;k]:padf[l+1;d]];.db.QX[z;k;l]:r[1];k:$[t=`0;`bsizeQ;`asizeQ];if[l>=count d:.db.QX[z;k];.db.QX[z;k]:padf[l+1;d]];.db.QX[z;k;l]:r[2]];t in `2`3`4`6`7`8`v;.db.QX[z;(`2`3`4`6`7`8`v!`price`price`open`settlepx`high`low`settlepx) t]:r[1];t=`z2;.db.QX[z;`openint]:r[2];()]}[ii;z] each (n,$[ii;10;19])#x;}; /竞价快照(Level-1 10[指数]/19[股票]byte定长记录,Level-2不适用)ii:isindex,t:买卖,r[3]档位 

decodexshgbinstream:{[x]l:0;y:x;while[1;if[24>m:count y;:l];h:(4 8 8 4;"sjji") 1: 24#y;if[(null t:h[0;0])|(0>n:h[3;0]);:-1];if[not t in key BINSS;lerr[`xshgmtype;(t;n)];:-2];if[m<n2:4+n1:24+n;:l];z:n1#y;y:n1 _y;c:4#y;if[not c~crc[z];:-3];y:4_y;l+:n2;z:24_z;if[.conf.dumpmsg;.temp.L,:enlist (.z.P;t;z)];@[BINSS[t];z;{[x;y;z].temp.m:y;linfo[`bin;(x;y;z)]}[t;z]]];}; /n2:完整消息长度,l:已成功解析字节数,y:剩余待解析字节流
...
//----ChangeLog----
//2023.11.24:新增BIN[309211i]占位函数,BIN[309011i]增加对n>0的校验
//2023.11.10:loadpcfsh/loadpcfsh增加金交所匹配
//2023.08.30:
//2023.08.24:快照行情处理函数BIN[300111i]和BINSS[`M102]增加对1档盘口的初始化以解决在涨跌停后1档盘口数据为过时数据的bug
//2023.06.29:初始版本
