...
FixTypeMap:`INT`FLOAT`CHAR`PRICE`BOOLEAN`QTY`AMT`SEQNUM!"IFCFCFFJ"; //fix数据类型到q语言数据类型的映射 
...
utctime:{$[`hs~.conf.fix.custom;-4_;::] toutc[.z.Z]};      /将当前时间格式化为FIX GMT时间戳字符串
