...
\d .temp
L2M:L2O:L2Q:QREF:QUEUE:L16:L15:L14:L13:L12:L11:L:C:();TKSub:BKSub:MDSub:()!();
\d .
...
.upd.DepthMD:{[x].temp.X:x;t:x[0];x:1_x;y:((.enum $[t;`XTPMDOpt;`XTPMDStk])!x),$[t;`yield_to_maturity`iopv!0n 0n;()];if[.conf.xtp.debug;.temp.L11,:enlist y];t0:sum dt:"DT"$0 8 cut string y`data_time;if[not `fqopendate in key .db;.db.fqopendate:0Nd];if[.db.fqopendate<d0:dt[0];pubm[`ALL;`MarketOpen;.conf.me;string d0];.db.fqopendate:d0];d:update virtualtime:0Np,isnormalsession:time within 08:00 16:00 from select sym:{[x;y]`$x,'".",/:string y}[ticker;.enum.exxtpQ exchange_id],time:dt[1],price:last_price,cumqty:`float$qty,vwap:turnover%`float$qty,high:high_price,low:low_price,bid:first each bp,ask:first each ap,bsize:first each bq,asize:first each aq,bidQ:bp,askQ:ap,bsizeQ:bq,asizeQ:aq,openint:settl_price,settlepx:iopv,open:open_price,pc:pre_close_price,sup:upper_limit_price,inf:lower_limit_price,mode:`$ticker_status,recvtime:.z.P,exlocaltime:.z.P,b1,a1,bm,am from enlist y;d:delete from d where (0>cumqty);if[count d;if[.conf.xtp.debug;.temp.L12,:d];d1:select sym,pc,open,sup,inf from d;if[n:count d2:d1 except .temp.QREF;pub[`quoteref;update refopt:n#enlist"" from d2];.temp.QREF,:d2];d3:select sym,bid,ask,bsize,asize,bm,am,b1,a1,extime:t0 from d where 0<bm|am;d:delete open,pc,sup,inf,b1,a1,bm,am,extime from d;n:count d;d2:select sym,bid,ask,bsize,asize,price,high,low,vwap,cumqty,openint,settlepx,mode,extime:t0,bidQ,askQ,bsizeQ,asizeQ,quoopt:n#enlist "" from d;$[1b~.conf.batchpub;enqueue[d2];pub[quotetbl;d2]]];if[0=count d3;:()];d4:(select sym,side:.enum.BUY,price:bid,size:bsize,num:`long$bm,qtyQ:b1,extime from d3),select sym,side:.enum.SELL,price:ask,size:asize,num:`long$am,qtyQ:a1,extime from d3;$[1b~.conf.batchpubl2;enqueuel2q[d4];pub[`l2queue;d4]];};
