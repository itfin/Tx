...
updxhkg:{[x;y]d:update esym:?[sectype=`WAR;`;ticker] from select sym:`$(,\:)[string sym;".XHKG"],ex:`SEHK,ticker:sym,sectype:?[secstatus=(`$"Derivative Warrants");`WAR;`STK],currency:`HKD,qtylot,secstatus,tradetype:name from update sym:`$string "F"$string sym from d1:select from (d0:`sym`name`secstatus`secclass`qtylot xcol ("SSSSFSSDSSSSSSSSFS";enlist "\t") 0: ssr[;",";""] each 2_system "wget -O /tmp/ListOfSecurities.xlsx http://www.hkex.com.hk/eng/services/trading/securities/securitieslists/ListOfSecurities.xlsx && xlsx2csv -d tab -i -s 1 /tmp/ListOfSecurities.xlsx") where secstatus in (`Equity,(`$"Exchange Traded Products"),(`$"Derivative Warrants"));nl:(exec sym from d) except exec sym from .db.QX;.db.QX:.db.QX uj 1!d;reqtwscon each exec sym from .db.QX where sym in nl,sectype=`STK;(path:` sv .conf.tempdb,.conf.me,`hklist) set d0;count nl}; /

updxhkgopt:{[x;y].temp.d:update underlying:`$(,\:)[string esym;".XHKG"] from update sym:`$(,\:)[string "I"$string sym;".XHKG"],esym:`$string "I"$string esym,putcall:?[`CALL=putcall;`C;`P],{"D"$"." sv reverse "-" vs x} each settledate from delete from (`sym`tradephase`suspendtype`esym`putcall`settledate`currency`strikepx xcol ("SSSSS*SF";enlist "\t") 0: -1_7_ system "wget -O - http://www.hkex.com.hk/eng/dwrc/search/dwAQList.csv|iconv -f UTF-16 -t ASCII") where `HKD<>currency;.db.QX:.db.QX lj 1!.temp.d;reqtwscon each exec sym from .db.QX where sectype=`WAR,not null esym,null ex1,null status;};
