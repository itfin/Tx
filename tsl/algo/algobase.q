...
/algo msg
.upd[`ExecRptAlgo]:{[x].temp.x00:x;x[`oid`msg]:(x`id;forcestring x`msg);if[.conf.me~y:x`sym;:()];z:key r:`sym`typ`oid`status`cumqty`avgpx`feoid`ordid`exchid`cstatus`cfeoid`corderid`cexchid`reason`msg`rptopt!(y;.enum`NEW;`;.enum`REJECTED;0f;0f;`;`;`;.enum`NULL;`;`;`;0N;"";"");w:key x;pub[`exerpt;enlist z#((z inter w)#x),(z except w)#r];};
.upd[`CxlRejAlgo]:{[x].temp.x00:x;if[.conf.me~x`sym;:()];pub[`cxlrej;enlist x];};

.upd[`NewOrderAlgo]:{[x].temp.x01:x;he:x`src;coid:x`id;k:exec first id from .db.O1 where clt=he,cltid=coid;$[not null k;.upd[`ExecRptAlgo][`sym`id`status`reason`msg!(he;coid;.enum`REJECTED;.enum`UNKNOWN_ORDER;"Already have order ",string coid)];[k:newid[];x[`qty`amt`price]:`float$x`qty`amt`price;if[not null a:x`amt;x[`qty]:roundqty[x`sym`side] a%get_last_price[x`sym]];.db.O1[k;`tsexec`ts`acc`acc1`ordid`status`ntime`cumqty`cumamt`cumfee`avgpx`lastqty`lastpx`sentqty`style`clt`algo`plan`snap`cltid`handlinst`execinst`sym`side`typ`qty`amt`price`para`cltalt`cltsub`cltid2`cltacc`ex`sym`stoppx`posefct]:(.conf.algots;.conf.algots^x`ts;x`acc;x`acc1;newid[];.enum`NEW;now[];0f;0f;0f;0f;0f;0f;0f;.db.Ts[.conf.algots;`DEFAULTSTYLE];he;x`algo;();()),x`id`handlinst`execinst`sym`side`typ`qty`amt`price`para`cltalt`cltsub`cltid2`cltacc`ex`sym`stoppx`posefct;.db.Ax[k]:.enum.nulldict;.db.Ax[k;`sids`ahead]:(();()!`symbol$());.upd[`ExecRptAlgo][`sym`id`status`ordid!.db.O1[k;`clt`cltid`status`ordid]]]];}; 

.upd[`CancelOrderAlgo]:{[x].temp.x02:x;he:(x`src)^x`clt;z:x`id;cid:x`cid;k:exec first id from .db.O1 where clt=he,cltid=z;$[null .db.O1[k;`sym];rejectcxloa[he;z;cid;.enum`UNKNOWN_ORDER_CXL;`Unknown];0=(-/).db.O1[k;`qty`cumqty];rejectcxloa[he;z;cid;.enum`TOO_LATE_TO_CANCEL;`Fill];.enum[`NULL]<>.db.O1[k;`cstatus];rejectcxloa[he;z;cid;.enum`ALREADY_PENDING;`Pending];[.db.O1[k;`cltcid`cstatus`cordid`ctime`pending`lastqty`lastpx]:(cid;.enum`PENDING_CANCEL;newid[];now[];1b;0f;0f);if[count t:forcestring x`msg;.db.O1[k;`msg]:t];execrptoa[k];$[.db.O1[k;`sentqty]=.db.O1[k;`cumqty];[.db.O1[k;`end`pending]:(1b;0b);$[0f<cxlqty:.db.O1[k;`qty]-.db.O1[k;`cumqty];[.db.O1[k;`status`cstatus]:(.enum`CANCELED;.enum`CANCELED);execrptoa[k]];[.db.O1[k;`cstatus]:.enum`REJECTED;rejectcxloa[he;z;cid;.enum`TOO_LATE_TO_CANCEL;`Fill]]]];cxloachildren[k]]]];};

.upd[`ReplaceOrderAlgo]:{[x].temp.x03:x;he:x`src;z:x`id;cid:x`cid;k:exec first id from .db.O1 where clt=he,cltid=z;if[null .db.O1[k;`sym];rejectrploa[he;z;cid;.enum`UNKNOWN_ORDER_CXL;`Unknown];:()];x[`qty`amt`price]:"f"$x`qty`amt`price;if[not null a:x`amt;x[`qty]:roundqty[x`sym`side] a%get_last_price[x`sym]];if[(.enum[`FILLED]=.db.O1[k;`status])&(not .db.O1[k;`qty]>x`qty);rejectrploa[he;z;cid;.enum`TOO_LATE_TO_CANCEL;`Fill];:()];if[.enum[`NULL]<>.db.O1[k;`cstatus];rejectrploa[he;z;cid;.enum`ALREADY_PENDING;`Pending];:()];if[(.db.O1[k;`cumqty]>x`qty)|(not .db.O1[k;`sym]~x`sym)|(not .db.O1[k;`side]~x`side)|(not .db.O1[k;`handlinst]~x`handlinst);rejectrploa[he;z;cid;.enum`BROKER_OPTION;`MismatchSymOrSideOrHandlInstOrQtyError];:()];mycid:newid[];.db.O1[k;`cid`cltcid`cstatus`cordid`pending`lastqty`lastpx]:(mycid;cid;.enum`PENDING_REPLACE;newid[];1b;0f;0f);execrptoa[k];.db.O1[k;`x0]:enlist x;$[.db.O1[k;`cumqty]<.db.O1[k;`sentqty];[cxloachildren[k]];[.db.O1[mycid;`ordid`status`ntime`plan`snap`cltid2`cltacc`tsexec`ts`acc`acc1`ex`sym`cltsub`cltalt`cumqty`cumamt`cumfee`avgpx`lastqty`lastpx`sentqty`style`suspend`previd`clt`algo`cltid`handlinst`execinst`sym`side`typ`qty`amt`price`para`stoppx`posefct]:(.db.O1[k;`cordid];$[0<.db.O1[k;`cumqty];.enum`PARTIALLY_FILLED;.enum`NEW];now[];();();x`cltid2;x`cltacc),.db.O1[k;`tsexec`ts`acc`acc1`ex`sym`cltsub`cltalt`cumqty`cumamt`cumfee`avgpx`lastqty`lastpx`sentqty`style],(((string x`ExecInst) like "*S*");k;he;x`algo),x`cid`handlinst`execinst`sym`side`typ`qty`amt`price`para`stoppx`posefct;.db.Ax[mycid]:.enum.nulldict;.db.Ax[mycid;`sids`ahead]:(();()!`symbol$());update upid:mycid from `.db.O where upid=k;.db.O1[k;`end`pending`status`cstatus`qty`amt`price`typ]:(1b;0b;.enum`REPLACED;.enum`REPLACED),x`qty`amt`price`typ;execrptoa[k]]];}; 

.upd[`NewOrderAlgoList]:{[x].temp.x04:x;he:x`src;coid:x`id;k:exec first id from .db.O1 where clt=he,cltid=coid;if[null k;k:newid[];.db.O1[k;`status`extype`slot`clt`cltid`cltalt`cltsub`cltid2`cltacc`algo`para]:(.enum`NEW;`LIST;x`totalsize;he;coid),x`cltalt`cltsub`cltid2`cltacc`algo`para;.db.Ax[k]:.enum.nulldict;.db.Ax[k;`full`sids`init]:(0b;();0b)];sids:{[listid;he;x]coid:x`id;k:exec first id from .db.O1 where clt=he,(cltid=coid)|(upid=listid)&(slot=x`slot);$[not null k;[.upd[`ExecRptAlgo][`sym`id`status`reason`msg!(he;coid;.enum`REJECTED;.enum`UNKNOWN_ORDER;"Already have order ",string coid)]];[k:newid[];x[`qty`amt`price]:"f"$x`qty`amt`price;if[not null a:x`amt;x[`qty]:roundqty[x`sym`side] a%get_last_price[x`sym]];.db.O1[k;`extype`upid`tsexec`ts`acc`acc1`ordid`status`ntime`cumqty`cumamt`cumfee`avgpx`lastqty`lastpx`sentqty`style`clt`algo`plan`snap`slot`cltid`handlinst`execinst`sym`side`typ`qty`amt`price`para`cltalt`cltsub`cltid2`cltacc`ex`sym`stoppx`posefct]:(`LISTCHILD;listid;.conf.algots;.conf.algots^x`ts;x`acc;x`acc1;newid[];.enum`NEW;now[];0f;0f;0f;0f;0f;0f;0f;.db.Ts[.conf.algots]`DEFAULTSTYLE;he;x`algo;();()),x`slot`id`handlinst`execinst`sym`side`typ`qty`amt`price`para`cltalt`cltsub`cltid2`cltacc`ex`sym`stoppx`posefct;.upd[`ExecRptAlgo][`sym`id`status`ordid!he,.db.O1[k;`cltid`status`ordid]]]];k}[k;he] each x`list;if[(not .db.Ax[k;`full])&(.db.O1[k;`slot]=max x[`list;;`slot]);.db.Ax[k;`full]:1b];if[0<count sids:sids except `;.db.Ax[k;`sids]:.db.Ax[k;`sids],sids];};

.upd[`CancelOrderAlgoList]:{[x].temp.x05:x;he:x`src;coid:x`id;cid:x`cid;k:exec first id from .db.O1 where clt=he,cltid=coid,extype=`LIST;if[null k;:rejectcxloa[he;coid;cid;.enum`UNKNOWN_ORDER_CXL;`UnknownListID]];if[.db.O1[k;`status]=.enum`CANCELED;:rejectcxloa[he;coid;cid;.enum`UNKNOWN_ORDER_CXL;`ListAlreadyCanceled]];if[.db.O1[k;`pending];:rejectcxloa[he;coid;cid;.enum`UNKNOWN_ORDER_CXL;`ListAlreadyCanceling]];.db.O1[k;`pending]:1b;y:.db.Ax[k;`sids];cxloasys[;x`msg] each y;};

.upd[`ListStatusAlgo]:{[x].temp.x10:x;k:x`listid;if[(not `LIST~.db.O1[k;`extype])|.db.O1[k;`end];:()];st:x`status;r:x`reason;m:x`msg;.db.O1[k;`status`reason`msg]:(st;r;m);if[st=.enum.REJECTED;rejectoa[;r;m] each .db.Ax[k;`sids];.db.O1[k;`end]:1b];};

listrejx:{[t;x;y;z].upd[`ListStatusAlgo][`sym`listid`type`status`reason`msg!(t;x;.enum`RESPONSE;.enum`REJECTED;y;forcestring z)];}; /[sym;listid;reason;text]
listrej:listrejx[.conf.me];
...
oa_cxl_cancel:{[k].db.O1[k;`end`pending]:(1b;0b);cxlqty:.db.O1[k;`qty]-.db.O1[k;`cumqty];if[`LISTCHILD=.db.O1[k;`extype];lst_cxl_child_end[k]];.db.Ax _:k;
 ...
 [.db.O1[k;`status`cstatus]:(.enum`FILLED;.enum`REJECTED);rejectcxloa[.db.O1[k;`clt];.db.O1[k;`cltid];.db.O1[k;`cltcid];.enum`TOO_LATE_TO_CANCEL;`Fill]] /rej
 ];};
...
ono_algo:{[x;y]k:.db.O[y;`upid];ag:.db.O1[k;`algo]; 
 ...
 if[.db.O[y;`status]=.enum[`REJECTED];.db.O1[k;`sentqty]-:.db.O[y;`qty];.db.O1[k;`jtime`jn]:(now[];1i+0i^.db.O1[k;`jn]);if[.db.O1[k;`jn]>=.db.Ts[.conf.algots]`MAXREJECT;info:`$"Auto Pause AlgoOrder For successive Rejected Child Orders.";$[`HS~.db.Ts[.conf.algots]`OMSTYPE;pauseoa[k;.enum`SEVERE;.enum`USERATTENTIONREQ;info];$[not null k1:.db.O1[k;`upid];cxloalst[k1;info];cxloasys[k;info]]]]; /子单被拒绝,当连续拒绝超过.db.Ts[.conf.algots][`MAXREJECT]时若OMS为恒生则主动暂停母单,若OMS为彭博则主动撤销母单(2012.03.26) ,string .db.O[y;`msg] 
 ...
 if[0<lqp[0];nq:qp[0]+lqp[0];m:getmultiple[.db.O[y;`sym]];ca:m*0f^((prd 0f^qp)+(prd 0f^lqp));ap:0f^ca%nq*m;lq:.db.O1[k;`qty]-nq;if[(`BB~.db.Ts[.conf.algots][`OMSTYPE])&(0f=roundqty[.db.O1[k;`sym`side]] lq);lq:0f];s:.enum[$[0f<lq;`PARTIALLY_FILLED;`FILLED]];
    .db.O1[k;`jn`ftime`cumqty`avgpx`cumamt`status`lastqty`lastpx]:(0i;now[];nq;ap;ca;s),lqp;if[s=.enum`FILLED;.db.O1[k;`end]:1b];execrptoa[k]; /当子单有成交时重置子单拒绝统计量,当买单零股不足100股时若开关IGNOREODDLOT打开则回完成(20120320)/直接回完成(20110506)
...
xnewsubord:{[h;texp;pfix;upid;id;qty;price;slot].temp.x_xn:(h;texp;pfix;upid;id;qty;price;slot);if[(qty<=0)|(price<0);lerr[`newsubord;(upid;`qtyorprice;`$"newsubord[",(string pfix),";",(string upid),";",(string id),";",(string qty),";",(string price),";",(string slot),"]";0)];:()];if[violateoddlotrule[upid;qty];lwarn[`newsubordcantsend;(upid;`oddlotmustsellallonce;.db.O1[upid;`side];(-/).db.O1[upid;`qty`cumqty];qty)];:()];if[.db.O1[upid;`typ] in .enum`LIMIT`STOP_LIMIT;plim:.db.O1[upid;`price];pmkt:getordpx[.db.O1[upid;`sym];.db.O1[upid;`side];`LEASTPASSIVE];if[((.db.O1[upid;`side]~.enum`BUY)&(price>plim))|((.db.O1[upid;`side]~.enum`SELL)&(price<plim));$[not pfix;price:plim;[linfo[`newsubordskip;(upid;`price_exceed_limit;.db.O1[upid;`side];price;plim;pmkt;pfix;qty;slot;id)];:()]]]];qtyadjust[upid;price];qty&:.db.O1[upid;`qty]-sq:0f^.db.O1[upid;`sentqty];k:newordex[.db.O1[upid;`side];.db.O1[upid;`posefct]^chfill h`posefct;.db.O1[upid;`ts];.db.O1[upid;`sym];qty;price;id;`acc`acc1`tsexec`tif`typ`upid`slot`style`expiretime`ex`esym`cltid2`cltacc`handlinst`msg`stoppx`cltalt`cltsub!(.db.O1[upid;`acc`acc1`tsexec]^' sfill each h`acc`acc1`tsexec),($[slot<0;.db.O1[upid;`tif];.enum`DAY];$[slot<0;.db.O1[upid;`typ];$[price=0f;.enum`MARKET;.enum`LIMIT]];upid;slot;.db.O1[upid;`style];(`timestamp$(`date$now[])+{$[(x>.conf.ex[`XSHG;`closeAM])&(x<.conf.ex[`XSHG;`openPM]);x+(.conf.ex[`XSHG;`openPM])-.conf.ex[`XSHG;`closeAM];x]} (`time$now[])+`time$.db.Ts[.conf.algots][`DEFAULTTIMEOUT]^"T"$cfill {$[99h=type x;x`CxlTimeout;`]} .db.O1[upid;`para][0])^texp;.db.O1[upid;`ex];.db.O1[upid;`esym];.db.O1[upid;`cltid2];.db.O1[upid;`cltacc];$[slot<0;.db.O1[upid;`handlinst];`];$[slot<0;`$cfill .db.O1[upid;`para][0;`Text];`];.db.O1[upid;`stoppx];.db.O1[upid;`cltalt];.db.O1[upid;`cltsub])];.db.O1[upid;`sentqty]:qty+sq;.temp.hO[k]:upid;k}; /创建新的小单:更新小单表,对柜台fix网关发NewOrderSingle.当母单有限价且子单报价超限时:若无报价只读限制则以限价报盘,否则放弃本次子单报盘. 2012.03.09去掉且限价在当前盘口有效(优于或等于本方最优价格)&(((.db.O1[upid;`side]~.enum`BUY)&(plim>=pmkt))|((.db.O1[upid;`side]~.enum`SELL)&(plim<=pmkt))) if[.db.O1[k;`typ]=.enum`LIMIT;plim:.db.O1[x;`price];if[((.db.O1[k;`side]=.enum`BUY)&())|((.db.O1[k;`side]=.enum`BUY)&());pnew:plim];if[ishidden[.db.O1[k;`sym];.db.O1[k;`side];pnew];:()]];

newsubord:xnewsubord[()!()];
...
rejectoa:{[x;y;z]if[null .db.O1[x;`sym];:()];.db.O1[x;`status`end`reason`msg]:(.enum`REJECTED;1b;y;tostring z);execrptoa[x];};
...
cxloasysx:{[c;k;t]if[.db.O1[k;`suspend]|.db.O1[k;`pending]|.db.O1[k;`end];:()];.upd[`CancelOrderAlgo][`clt`id`cid`msg!(.db.O1[k;`clt];.db.O1[k;`cltid];c;t)];};
cxloasys:cxloasysx[`];

cxlalloa:{[]cxloasys[;`manualcxlall] each exec id from .db.O1 where not end,status<>"2";};

rejectcxlrploa:{[src;id;cid;reason;msg;isrpl].upd[`CxlRejAlgo][`sym`oid`cid`cstatus`cordid`reason`msg`isrpl!(src;id;cid;.enum`REJECTED;newid[];reason;forcestring msg;isrpl)];};
...
newalgord:{[x;y;z;t;p;a;u;v]e:`;pe:$[2<count x;x[2];.enum`NULL];if[1<count x;e:x[1];x:x[0]];ts:ac:to:`;if[1<count a;to:a[0];ac:a[1];ft:a[3];k:a[4];ts:a[5];a:a[2]];if[(p=0f)&(t=.enum`LIMIT);t:.enum`MARKET];if[null k;k:newid[]];.upd[`NewOrderAlgo][`src`id`ts`acc`acc1`handlinst`execinst`sym`side`typ`qty`amt`price`algo`para`cltalt`cltsub`cltid2`cltacc`ex`sym`stoppx`posefct!(.conf.me^ft;k;to;ac;a;.enum`AUTOMATED_EXECUTION_ORDER_PUBLIC_BROKER_INTERVENTION_OK;e;x;y;t;z;0n;p;u;enlist v;ts;`;`;`;fs2e x;fs2s x;$[t=.enum`STOP;p;0n];pe)];k}; /[sym|(sym;execinst)|(sym;execinst;posefct);side;qty;ordtype;price;acc1|(tsorig;acc;acc1;ft;oid;ts);algo;parahash]

newalgolst:{[x;y;z;t;p;a;u;v].upd[`NewOrderAlgoList][`id`totalsize`algo`para`list!(newid[];-1+count x;u;enlist v;{[s;x;y;z;t;p;a]e:`;pe:$[2<count x;x[2];.enum`NULL];if[1<count x;e:x[1];x:x[0]];ac:to:`;if[1<count a;to:a[0];ac:a[1];a:a[2]];`src`id`ts`acc`acc1`handlinst`execinst`sym`side`typ`qty`amt`price`cltalt`cltsub`cltid2`cltacc`ex`sym`slot`algo`para`stoppx`posefct!(.conf.me;newid[];to;ac;a;.enum`AUTOMATED_EXECUTION_ORDER_PUBLIC_BROKER_INTERVENTION_OK;e;x;y;t;z;0n;p;`;`;`;`;fs2e x;fs2s x;s;`;();$[t=.enum`STOP;p;0n];pe)}'[til count x;x;y;z;t;p;a])];}; /[sym|(sym;execinst);side;qty;ordtype;price;acc;algo;para]
...
//2024.10.15:新增在途子单计数函数nsuboids
//2024.10.14:新增.upd[`ListStatusAlgo]和listrejx以支持篮子算法单拒绝,更新rejectoa以修复
//2024.10.09:ono_algo增加对multiplier的处理以支持衍生品
//2024.09.11:newalgord/newalgolst的a入参为数组时调整为(tsorig;acc;acc1;ft;oid;ts)以支持期货净持仓算法
//2024.08.13:.upd[`CancelOrderAlgo]修复空msg字段时类型错误的bug,同时在.upd[`ExecRptNOE]中增加保护
//2024.06.14:ono_algo增加当母单完全成交时置end为1b的逻辑
//2024.06.10:将newsubord扩展为xnewsubord以支持子单使用与母单不同的posefct/acc/acc1,更新.upd`ExecRptAlgo/CxlRejAlgo/NewOrderAlgo及rejectcxlrploa/oa_cxl_cancel/cxloasysx/newalgord以支持从上游创建母单并向上游ft/(fix)oms发母单成交回报
//2024.01.18:newalgord/newalgolst的a入参为数组时调整为(ts;acc;acc1)以支持代其他多账户策略做算法交易,对应修改.upd[`NewOrderAlgo]/.upd[`ReplaceOrderAlgo]/.upd[`NewOrderAlgoList]/newsubord
//2023.10.23:checkqs改成从sym计算ex
