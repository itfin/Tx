...
otchedgereq=function(x,y){wscall(['{[x]y:.db.Ts[x;`UDL]^sfill .db.Ts[x;`HedgeUDL];z:otc_hedgemap[x];g:(1_ key z)!1_ (deltas value z)*(key z)%(deltas key z)*100;traderlst:x;q:otc_hedgepos[x];p:otc_hedgepx[x];m:1f^.db.QX[y;`multiplier];fq:exec sum ((0f^lqty)+0f^sqty) from .db.P where ts in traderlst,sym<>y;bias:q-z[p];e:0.01*til 11;md:0<count z1:.db.Ts[x;`HedgeMapList];if[md;z0:map_hedgeoffset[x];d0:select from (select sym,nqty:(0f^lqty)+0f^sqty from .db.P where ts=x,isopt each sym) where 0<abs nqty;mf:$[1b~.db.Ts[x;`UseCache];{[x; y] opt_deltamapc[x;y][`HedgeMapList]}[x];{[x;y]`s#{where[not null[x[;0]]]#x} {(last flip key x)!value[x]} (y,/:key[.db.Ts[x;`HedgeMapList]])#.db.Ts[x;`OptHedgeMapList]}[x]]; z1:`s#({`s#(x+key y)!value y}[z0] z1)+$[(1b~.db.Ts[x;`UseOpt])&count d0;`s#exec sum nqty*mf each sym from d0;0f]];`p`t`ON`X`Y`Y0`fardelta`delta`gamma`ddelta`pos`bias`udl`active`neggamma`mode`md`dL`warn`data`optdelta!(p;z[p];exec sum cumqty from .db.O where sym like ((2#string y),"*"),posefct=.enum`OPEN;.math.r2 1e-4*abs bias*m*p;.math.r2 1e-4*1e6|abs[0.1*(q+fq)*m*p]|abs g[p]*p*2*m;`q`fq`m`p`g!(q;fq;m;p;g[p]);fq;flip (key z;value z);flip (key g;value g);flip (1e2*e;{p:key x;avg (p where (p<=y*1+0.5*z)&(p>=y*1-0.5*z))#x}[z;p] each e);enlist (`float$$[0>=p;.db.QX[y;`pc];p];0f)^p,q;.math.r2 bias;y;.db.Ts[x;`active];isneggamma[x];.db.Ts[x;`mode];md;$[md;(key z1),\'/:flip value z1;()];$[1b~.db.Ts[x;`BasketUDL];`$"Basket,STOP@",sv[","] string exec sym from ((select from .db.ETFPF where etfsym=y) lj select last price by sym from .db.QX) where price<=0;`];flip `price`pos`gamma!(key g;1_value z;value g);$[1b~.db.Ts[x;`UseOpt];otc_optmap[x] p;0f])}',x],otchedgeres,{'target':'plot'});} // 支持场内期权cache版本
...
//基差监控
basismonreq=function(){wscall(['{[](enlist `data)!enlist `sym xasc update basisyearly:100*(basis%remainingyears)%idxprice,`$string settledate from update basis:price-idxprice,remainingyears:(settledate-.z.D)%365 from (select sym,price,settledate-2,product from .db.QX where ex=`CCFX,cumqty>0,product in `IF`IC`IH`IM) lj 1!select product:(`000300.XSHG`000016.XSHG`000905.XSHG`000852.XSHG!`IF`IH`IC`IM) sym,idxsym:sym,idxprice:price from .db.QX where sym in `000300.XSHG`000016.XSHG`000905.XSHG`000852.XSHG}','`'],basismonres,{'t0':'ctrl','t1':'grid','t2':'plot'});}
basismonres=function(x,y){
   $('#'+x.t1).html('<div id=csvgrid>');
    $('#csvgrid').datagrid({fit:true,singleSelect:true,remoteSort:false,pagination:false,columns:[[{field:'sym',title:'期货代码',width:120,sortable:true},{field:'price',title:'期货价格',width:120,sortable:true},{field:'settledate',title:'结算日',width:120,sortable:true},{field:'product',title:'产品代码',width:60,sortable:true},{field:'idxsym',title:'指数代码',width:120,sortable:true},{field:'idxprice',title:'现货价格',width:120,sortable:true},{field:'basis',title:'基差',width:120,sortable:true,formatter:function(x){return x.toFixed(4);}},{field:'remainingyears',title:'剩余年限',width:120,sortable:true,formatter:function(x){return x.toFixed(4);}},{field:'basisyearly',title:'年化基差(%)',width:120,sortable:true,formatter:function(x){return x.toFixed(4);}}]],rowStyler:function(index,row){if((row.product=='IF')||(row.product=='IM')){return 'background-color:#6293BB;color:#fff;';}}});
    $('#csvgrid').datagrid('loadData',{total:y.data.length,rows:y.data});
};
...
ngxres=function(x,y){
    ...
    $('#'+x.t1).html('频率:<input type=radio name=btyp value=C onclick="chgbtyp()">连续合约日线<input type=radio name=btyp value=D checked onclick="chgbtyp()">日线<input type=radio name=btyp value=M onclick="chgbtyp()">分钟线,</input>起始日期:<input type=text id=ngxd0 name=ngxd0 style="width:150px"></input>,</input><input type=button id=d0prev value="<-" onclick="decd0()"></input></input><input type=button id=d0d1prev value="<=" onclick="decd0d1()"></input></input><input type=button id=d1prev value="-<" onclick="decd1()"></input><input type=text id=dmstep value=1 style="width:50px"><input type=button id=d1next value="->" onclick="incd1()"></input><input type=button id=d0d1next value="=>" onclick="incd0d1()"></input><input type=button id=d0next value=">-" onclick="incd0()"></input>,结束日期:<input type=text id=ngxd1 name=ngxd1 style="width:150px">去尾:<input type=text id="droptail" value=0 style="width:80px">,<input type=button id=dtprev value="<-" onclick="incdt()"></input><input type=text id=dtstep value=5 style="width:50px"><input type=button id=dtnext value="->" onclick="decdt()"></input>,截头:<input type=text id="drophead" value=0 style="width:80px">,<input type=button id=dhprev value="<-" onclick="incdh()"></input><input type=text id=dhstep value=5 style="width:50px"><input type=button id=dhnext value="->" onclick="decdh()"></input>,显示:<input type=radio name=ptyp value=K checked  onclick="ngxupd()">K线(分划:<input type=text id="cutpara" value="0.2 5" style="width:80px">, <input type=checkbox name=strd value="T" onclick="ngxupd()"/>显示交易)<input type=radio name=ptyp value=L onclick="ngxupd()">轮廓<input type=button id=ngbuy value="买入" onclick="ngbuy()"></input><input type=text id=ngpx value=0 style="width:80px"><input type=button id=ngsell value="卖出" onclick="ngsell()"></input><br><div id=ngxlst style="align:center;width:'+dw+'px;height:'+dh+'px">');$('#'+x.t2).html('<div id=ngxflot style="width:600px;height:300px">');
    $('#ngxlst').datagrid({fit:false,singleSelect:true,remoteSort:false,pagination:false,columns:[[{field:'sym',title:'期货合约',width:80,sortable:true},{field:'product',title:'品种',width:30,sortable:true},{field:'name',title:'名称',width:100,sortable:true,formatter:function(x){return togbk(x);}},{field:'nend',title:'夜盘结束',width:60,sortable:true},{field:'cumamt',title:'成交金额',width:60,sortable:true,formatter:function(x){return x.toFixed(2);}},{field:'size',title:'合约大小',width:60,sortable:true,formatter:function(x){return x.toFixed(2);}},{field:'feeunit',title:'手续费(%)',width:60,sortable:true,formatter:function(x){return x.toFixed(2);}},{field:'pos',title:'持仓',width:60,sortable:true},{field:'cost',title:'成本',width:60,sortable:true},{field:'price',title:'现价',width:60,sortable:true},{field:'pnl',title:'盈亏',width:60,sortable:true},{field:'lcpx',title:'多头止损',width:60,sortable:true},{field:'scpx',title:'空头止损',width:60,sortable:true},{field:'dtr',title:'日趋势',width:50,sortable:true},{field:'dS1',title:'日支撑1',width:80,sortable:true},{field:'dS0',title:'日支撑0',width:80,sortable:true},{field:'dR0',title:'日阻力0',width:80,sortable:true},{field:'dR1',title:'日阻力1',width:80,sortable:true},{field:'tr',title:'分趋势',width:50,sortable:true},{field:'s1',title:'分支撑1',width:80,sortable:true},{field:'s0',title:'分支撑0',width:80,sortable:true},{field:'r0',title:'分阻力0',width:80,sortable:true},{field:'r1',title:'分阻力1',width:80,sortable:true},{field:'ltr',title:'前分趋势',width:50,sortable:true},{field:'ls1',title:'前分支撑1',width:80,sortable:true},{field:'ls0',title:'前分支撑0',width:80,sortable:true},{field:'lr0',title:'前分阻力0',width:80,sortable:true},{field:'lr1',title:'前分阻力1',width:80,sortable:true}]],onClickRow:function(index,r){ngxsymreq(r.sym);}}); //,{field:'pc',title:'昨收',width:60,sortable:true},{field:'ph',title:'昨高',width:60,sortable:true,formatter:function(x){return x.toFixed(2);}},{field:'pl',title:'昨低',width:60,sortable:true},{field:'pq0',title:'昨集合成交',width:80,sortable:true},{field:'pq',title:'昨成交',width:60,sortable:true},{field:'h5',title:'5日高点',width:80,sortable:true},{field:'l5',title:'5日低点',width:60,sortable:true},{field:'qm',title:'5日均量',width:60,sortable:true},{field:'q0m',title:'集合竞价均量',width:60,sortable:true}
...
ngorder=function(x,y){$("input[name=ufreq][value=0]").prop("checked","checked");var r=$('#ngxlst').datagrid('getSelected');if(r==undefined)return;s=r.sym;p=$('#ngpx').val();if(!confirm('确认'+((x>0)?'买入':'卖出')+s+'@'+p+'?')) return;wscall(['{[x;y]y:"F"$y;'+((x>0)?'nb':'ns')+'[x,y]}','`'+s,p],ngorderres,{})};
