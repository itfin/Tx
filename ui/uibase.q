.module.uibase:2024.11.08;

\d .h
ty[`htm]:"text/html;charset=gbk"; 
k).z.ph:{BH::x[1];x:uh$[@x;x;*x];if[(#x)>n:x?"?";x:n#x];$[~#x;hy[`htm]fram[$.z.f;x]("?";"?",*x:$."\\v");x~,"?";hp@{hb["?",x]x}'$."\\v";"?["~2#x;hp jx["J"$2_x]R;"?"=*x;@[{hp jx[0j]R::1_x};x;he];"="=*x;@[{hy[`htm]@. 1_x};x;he];"?"in x;@[{hy[t]@`/:tx[t:`$-3#n#x]@.(1+n:x?"?")_x};x;he];#r:@[1::;`$":",p:HOME,"/",x:(x?"?")#x;""];hy[`$(-$[(#x)>k:(|x)?".";k;0])#x]"c"$r;hn["404 Not Found";`txt]p,": not found"]}; 
k)hta:{,/("<";$x;,/" ",'($!y),'?[`checked=!y;(#y)#"";(#y)#"="],'{$[10h=abs[@x];"\"",x,"\"";$x]}'. y;">")};

k)css:{[x]""/: {hta[`link;`rel`type`href!("stylesheet";"text/css";x)]}' x};
k)jsload:{""/: {htac[`script;`type`src!("text/javascript";x);""]}' x};
k)jscode:{htac[`script;(,`type)!,"text/javascript";x]};
Td:htc[`td];Tr:htc[`tr];Th:htac[`th];Thead:htc[`thead];Tbody:htc[`tbody];Tbl:htac[`table];

cdata:{"<![CDATA[",x,"]]>"};
k)jv:{[x]$[10=abs t:@x;"'",x,"'";-11h=t;$x;-1h=t;$[x=1b;"true";"false"];-14h=t;"new Date (",($ `year$x),",",($ -1+`mm$x),",",($ `dd$x),")";-15h=t;"new Date (",($ `year$x),",",($ -1+`mm$x),",",($ `dd$x),",",($ `hh$x),",",($ `uu$x),",",($ `ss$x),",",($ {x-y*x div y}[;1000] `int$`time$x),")";-12h=t;jv "z"$x;-13h=t;jv "d"$x;-19h=t;jv .z.D+x;t in -17 -18h;jv `time$x;$x]}; /value to javascript

k)xv:{[x]$[99=t:@x;""/: (!x){[x;y]$[99=t:@y;htac[x;`.DATA. _y;xv y`.DATA.];(~0>t)&(~10=t);""/: ('[htc[x];xv])' y;htc[x] xv[y]]}' (. x);(0h=t)|(98h=t);""/: xv' x;-1h=t;$[x=1b;"true";"false"];10h=t;x;$x]}; /value to xml

input:htac[`input;;""];
button:{input[`type`value`onclick!(`button;x;y)]};

k)jsfunc:{($x),"=function(",(","/:$$[0>@y;,y;y]),"){",z,"};"};

//;"js/swfobject.js";"js/amstock.js""js/lib/jquery.flot.min.js";"js/lib/jquery.flot.time.min.js";"js/lib/jquery.flot.categories.min.js";"js/lib/jquery.flot.pie.min.js";"js/lib/jsxgraphcore.js";"js/lib/d3.v2.min.js";

k)txuimain:{[x;y]htc[`html](htc[`head] (htc[`title] .conf.ui.title),css[("/themes/default/easyui.css";"themes/icon.css";"css/codemirror.css";"css/jsxgraph.css";"css/mycss.css")],jsload[("js/lib/functional.min.js";"js/lib/jquery.min.js";"js/lib/jquery.easyui.min.js";"js/lib/jquery.json.min.js";"js/lib/jquery.timers.js";"js/lib/echarts.min.js";"js/lib/codemirror.js";"js/lib/jquery.flot.min.js";"js/lib/jquery.flot.time.min.js";"js/lib/jquery.flot.categories.min.js";"js/lib/swfobject.js";"js/lib/amstock.js";"js/lib/jsxgraphcore.js";"js/lib/d3/d3.v7.min.js";"js/c.js";"js/gbk.js";"js/tx/txlib.js";"js/tx/",.conf.ui.lib)],jscode["var app='",($ .conf`app),"';var me='",($ .conf`me),"';var user='",($ .z.u),"';var host='",(* ":" \: .h.BH`Host),"';wsUri='ws://'+'",(.h.BH`Host),"';$(function(){initws(wsUri);mkmenu(",($x),");});$(document).everyTime(1000,function(i){$('#clock').html(now());var x=$('input[name=ufreq]:checked').val();if((x==undefined)||(x==0))return;if(0==i%x){recall();$('#updtime').val(now());}});"]),algobody:htac[`body;(`class`id)!("easyui-layout";"txlayout")]htac[`div;(`id`region`collapsible`border`style)!(`top;"north";"false";"false";"height:25px;background:#B3DFDA;margin:0 0 0 0px");htac[`div;(`id`style)!(`autoupd;"width:600px;float:left;margin:0 0 0 0px");.conf.ui.title],htac[`div;(`id`style)!(`toolbar;"");""],htac[`div;(`id`style)!(`info;"width:200px;float:right;margin:0 0 0 0px;text-align:right;");"用户ID:[",($ .z.u),"]."]],htac[`div;(`id`region`collapsible`split`title`style)!(`left;"west";"true";"true";"功能导航";"width:200px;");htac[`ul;(,`id)!,`menu;""]],htac[`div;(`id`region`collapsible`border`style)!(`bot;"south";"false";"false";"height:25px;background:#A9FACD;");htac[`div;(`id`style)!(`tip;"width:300px;float:left;");"版本号:0.01"],htac[`div;(`id`style)!(`output;"");""],htac[`div;(`id`style)!(`clock;"width:60px;float:right;");""]],htac[`div;(`id`region`class,`$"data-options")!(`main;"center";"easyui-layout";"fit:true");(htac[`div;(`id`region`collapsible`border`style)!(`ctrl;"north";"false";"true";"height:30px;");""],htac[`div;(`id`region`collapsible`border`split`style)!(`plot;"south";"false";"false";"true";"height:350px;");""],htac[`div;(`id`region`border,`$"data-options")!(`grid;"center";"false";"fit:true");""])]}; /,htac[`div;(`id`region`collapsible`split`title`border`style)!(`right;"east";"false";"false";"";"false";"width:0px;");""]

txhome:txuimain[0];
\d .

.ctrl.wsconn:.enum.nulldict;

sendws:{[x;y]neg[x] -8!y;};

.z.wo:{[x].temp.zwo,:enlist (.z.P;x);};

.z.ws:{[x].temp.zws,:enlist (.z.P;x);h:.z.w;who:`$"ws_",string h;if[not h~.ctrl.wsconn[who;`h];.ctrl.wsconn[who]:.enum`nulldict;.ctrl.wsconn[who;`h`selfseq`peerseq]:(h;0N;0N)];y:-9!x;.temp.ws:(h;x;y);if[y[0]~`uim;z:y[2];if[10h=type z;y[2]:value z];if[(0h=type z)&(2=count z)&(count[first z]=count[last z]);y[2]:z[0]!z[1]];y[2]:(`who`seq!(who;0N)),y[2]];if[not (::)~r:value y;sendws[h;r]];}; /show (h;y);

.uim.WebReg:{[x]h:.z.w;y:x`who;.ctrl.wsconn[y;`h`proto`name`role`status`conntime`ipaddr`x0`selfseq`peerseq]:(h;`ws;x`name;x`role;`Registered;.z.P;`$"." sv string "i"$0x0 vs .z.a;{$[0h=type x;x;0h<type x;(x;::);null x;();(x;::)]}x`x0;0j^.ctrl.wsconn[y;`selfseq];(x`seq)^.ctrl.wsconn[y;`peerseq]);smws[y;`RegAck;(enlist `status)!enlist `RegOK];};

.uim.WebCall:{[x]y:x`who;smws[y;`CallRes;`cid`res!(x`cid;value x`exp)];};

uim:{[x;y]t0:.z.P;z:y`who;w:y`seq;r:.[{[x;y].uim[x;y]};(x;y);`trap];($[`trap~r;lerr;ldebug])[`uim;(z;w;x;r;count y`data;1e-6*`long$.z.P-t0)];};

smws:{[x;y;z].temp.smws:(x;y;z);if[not x in key .ctrl.wsconn;lwarn[`SmWsNoPeer;x];:()];h:.ctrl.wsconn[x;`h];if[0>h;if[not 1b~.ctrl.wsconn[x;`errmode];.ctrl.wsconn[x;`errmode]:1b;lwarn[`SmWsNoLink;(x;h)]];:()];t0:.z.P;u:0N;if[0<h;.ctrl.wsconn[x;`selftime];now[]];z:(`who`seq!(.conf.me,u)),`who`seq _ z;sendws[h;(`uim;y;z)];0}; /[target;mess;body] 

.z.wc:{[x].temp.zwc,:enlist (.z.P;x);{if[x=.ctrl.wsconn[y;`h];.ctrl.wsconn[y;`h`disctime]:(-1;.z.P)]}[x] each tkey .ctrl.wsconn;};

pubws:{[r;y]{[x;r;y]z:.ctrl.wsconn[x];if[(`Registered=z`status)&(r=z`role)&(0<h:z`h);sendws[h;y]];}[;r;y] each tkey .ctrl.wsconn;}; /[role;mess]

mplot:{pubws[`Monitor;(`Plot;x)];};mgrid:{pubws[`Monitor;(`Grid;x)];};jplot:{pubws[`Monitor;(`Pd;x)];}; 

//----ChangeLog----
//2024.11.08:txuimain增加js变量user定义
//2024.07.25:txuimain新增在js里定义me变量
//2024.06.17:txuimain新增加载d3库
//2019.01.08:初始版本